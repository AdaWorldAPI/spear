...
/// Create a RecordBatchIterator from an empty batch (for table initialization)
fn empty_reader(schema: arrow::datatypes::Schema) -> RecordBatchIterator<std::vec::IntoIter<std::result::Result<RecordBatch, arrow::error::ArrowError>>> {
...
pub async fn init_tables(db: &Connection) -> Result<()> {
    ...
    if db.open_table("messages").execute().await.is_err() {
        db.create_table("messages", empty_reader(messages::schema()))
            .execute()
            .await
    ...
    if db.open_table("folders").execute().await.is_err() {
        db.create_table("folders", empty_reader(folders::schema()))
            .execute()
            .await
            .map_err(|e| Error::Lance(e.to_string()))?;
    ...
    if db.open_table("accounts").execute().await.is_err() {
        db.create_table("accounts", empty_reader(accounts::schema()))
            .execute()
            .await
            .map_err(|e| Error::Lance(e.to_string()))?;
    ...
    if db.open_table("events").execute().await.is_err() {
        db.create_table("events", empty_reader(events::schema()))
            .execute()
            .await
            .map_err(|e| Error::Lance(e.to_string()))?;
    }
    ...
    if db.open_table("contacts").execute().await.is_err() {
        db.create_table("contacts", empty_reader(contacts::schema()))
            .execute()
            .await
            .map_err(|e| Error::Lance(e.to_string()))?;
    }
